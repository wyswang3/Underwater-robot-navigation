...
struct Stamp {
    ...
    // 对外统一使用的时间戳（给 ESKF / Logger / NavState）：
    // 当前实现通常为 host_time_ns - latency_ns。
    // 若将来引入基于 sensor_time_ns 的时钟对齐，也在此字段体现。
...
//   frame.mono_ns = st.host_time_ns;
//   frame.est_ns  = st.corrected_time_ns;
//
