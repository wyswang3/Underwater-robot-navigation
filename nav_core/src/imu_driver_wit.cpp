...
#include <nav_core/imu_driver_wit.hpp>
...
void ImuDriverWit::onRegUpdate(std::uint32_t start_reg, std::uint32_t num) {
    ...
    raw.host_mono_ns = 0;                // 如有需要，可在此处改成 timebase::now_ns()
...
void ImuDriverWit::makeFrameFromRegs(std::uint32_t /*start_reg*/,
    ...
    // -------- 过滤逻辑 --------
    const ImuFilterConfig cfg = filter_cfg_;  // 拷贝一份，避免被其他线程修改时产生撕裂
    ...
    out.est_ns  = ts.corrected_time_ns;   // 统一后的“估计时间基”（含延迟补偿）
