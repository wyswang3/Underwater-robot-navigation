...
- 回调：把“原始 + 滤波 + 航向角 + 原始扩展字段”打包回传上层（不阻塞驱动回调）
"""
...
def _vec3_from_keys(d: dict, k0: str, k1: str, k2: str) -> Optional[np.ndarray]:
    """
    从 dict 中读取 3 个键，若都能转 float，则返回 np.ndarray(3,)；否则 None。
    不抛异常，保证底层回调安全。
    """
    x0 = _to_float(d.get(k0))
    x1 = _to_float(d.get(k1))
    x2 = _to_float(d.get(k2))
...
@dataclass
class IMUSample:
    """线程间传递的最小单元（原始 IMU 数据 + 时间戳 + 原始扩展字段）"""
    t_unix: float                # Unix时间戳（秒）
    t_mono: float                # 单调时钟（秒），用于估计 dt
    acc: np.ndarray              # (3,) 加速度（AccX/Y/Z）
    gyr: np.ndarray              # (3,) 角速度（AsX/Y/Z，deg/s）
    mag: Optional[np.ndarray]    # (3,) 磁场（HX/Y/Z），可选
    euler_deg: Optional[np.ndarray]  # (3,) 姿态角（AngX/Y/Z），单位度，可选
    temperature_c: Optional[float]   # 温度（°C），可选


@dataclass
class IMUResult:
    """传给上层（回调/上游模块）的数据包：原始 + 滤波 + 航向角 + 原始报文扩展"""
    t_unix: float
    acc_raw: np.ndarray              # (3,)
    gyr_raw: np.ndarray              # (3,) deg/s
    ...
    # ====== 原始报文“全量”字段（可选，不破坏旧逻辑）======
    mag_raw: Optional[np.ndarray] = None       # (3,)  -> HX,HY,HZ
    euler_deg: Optional[np.ndarray] = None     # (3,)  -> AngX,AngY,AngZ（Roll,Pitch,Yaw，度）
    temperature_c: Optional[float] = None      # -> Temperature（°C）
...
class IMUReader:
        ...
        if csv_dir:
            ...
            if self._csv_raw.tell() == 0:
                self._raw_writer.writerow([
                    "Timestamp(s)",
                    "AccX", "AccY", "AccZ",
                    "AsX(deg/s)", "AsY(deg/s)", "AsZ(deg/s)",
                    "HX", "HY", "HZ",
                    "AngX(deg)", "AngY(deg)", "AngZ(deg)",
                    "Temperature(C)"
                ])

            # FILT：保持滤波输出为主（Raw 对比用字段已在 raw csv）
            if self._csv_flt.tell() == 0:
                self._flt_writer.writerow([
                    "Timestamp(s)",
                    "AccX_f", "AccY_f", "AccZ_f",
                    "AsX_f(deg/s)", "AsY_f(deg/s)", "AsZ_f(deg/s)",
                    "Yaw(deg)"
                ])
    ...
    def _on_device_sample(self, data: dict) -> None:
        ...
        # 可选字段：磁场 / 欧拉角 / 温度
        mag = _vec3_from_keys(data, "HX", "HY", "HZ")  # 可能某些帧没有
        euler_deg = _vec3_from_keys(data, "AngX", "AngY", "AngZ")  # (deg)
        ...
        while not self._stop_evt.is_set():
            ...
            if self._raw_writer:
                ...
                self._raw_writer.writerow([
                    ...
                    *res.acc_raw.tolist(),
                    *res.gyr_raw.tolist(),
                    "" if mag is None else mag[0],
                    "" if mag is None else mag[1],
                    "" if mag is None else mag[2],
                    "" if eul is None else eul[0],
                    "" if eul is None else eul[1],
                    "" if eul is None else eul[2],
                    "" if res.temperature_c is None else res.temperature_c,
                ])
...
if __name__ == "__main__":
    def _printer(res: IMUResult):
        ...
        if res.yaw_deg is not None:
            print(f"{res.t_unix:.6f} | yaw={res.yaw_deg:7.3f}° "
                  f"| acc_raw={np.round(res.acc_raw,3)} gyr_raw={np.round(res.gyr_raw,3)}")
        else:
            print(f"{res.t_unix:.6f} | (calibrating...) "
                  f"| acc_raw={np.round(res.acc_raw,3)} gyr_raw={np.round(res.gyr_raw,3)}")
